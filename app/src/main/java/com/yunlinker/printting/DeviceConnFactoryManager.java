        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case READ_DATA:
                    ...
                    String status = myApp.getContext().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            ...
                            if (result == 0) {//打印机状态查询
                                ...
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " "+ myApp.getContext().getString(R.string.str_printer_out_of_paper);
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " "+ myApp.getContext().getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " "+ myApp.getContext().getString(R.string.str_printer_error);
                                }
                                System.out.println(myApp.getContext().getString(R.string.str_state) + status);
                                String mode= myApp.getContext().getString(R.string.str_printer_printmode_esc);
                                Utils.toast(myApp.getContext(), mode+" "+status);
                            }
                        }
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {//查询打印机实时状态
                                ...
                                System.out.println(myApp.getContext().getString(R.string.str_state) + status);
                                String mode= myApp.getContext().getString(R.string.str_printer_printmode_tsc);
                                Utils.toast(myApp.getContext(), mode+" "+status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            if (cnt == 1) {
                                ...
                                String mode= myApp.getContext().getString(R.string.str_printer_printmode_cpcl);
                                Utils.toast(myApp.getContext(), mode+" "+status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
